<template>
  <div class="demo-list-box" id="demo_list_box" :style="{height: `${height}px`}">
    <flexbox :gutter="0" v-for="(list, index) in components" :key="index" class="b_b_g">
      <flexbox-item :span="1/3" v-for="(component, index1) in list" :key="index1" class="cbox vux-1px-t vux-tap-active" @click.native="go(component.url)">
  ...
  methods: {
    go (name) {
      if (name === '' || name === undefined || name === null) {
        ...
        this.$router.push({path: `/mobile/${name}`})
      }
    },
    split (array) {
      let chunks = []
